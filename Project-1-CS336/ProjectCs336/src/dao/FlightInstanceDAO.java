...
public class FlightInstanceDAO {
    private Timestamp combineDateAndTime(Date date, Time time) {
        if (date == null) {
            return null;
        }
        if (time == null) {
        ...
        return Timestamp.valueOf(date.toString() + " " + time.toString());
    }
    ...
    public int createFlightInstance(FlightInstance instance) throws SQLException {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            pstmt.setTimestamp(4, combineDateAndTime(instance.getDepartureDate(), instance.getActualDepartureTime()));
            pstmt.setTimestamp(5, combineDateAndTime(instance.getDepartureDate(), instance.getActualArrivalTime()));
            pstmt.executeUpdate();
    ...
    public List<FlightInstance> getAllFlightInstances() throws SQLException {
        List<FlightInstance> instances = new ArrayList<>();
        String sql = "SELECT fi.*, f.departure_AirportID, f.arrival_AirportID, " +
                     "ac.capacity, " +
                     "(ac.capacity - COALESCE((SELECT COUNT(*) FROM includes i " +
                     ...
                     "JOIN Flight f ON fi.AirlineID = f.AirlineID AND fi.flight_number = f.flight_number " +
                     "JOIN Aircraft ac ON f.aircraftID = ac.aircraftID " +
                     "ORDER BY fi.departure_date DESC, fi.AirlineID, fi.flight_number";
    ...
    public boolean updateFlightInstance(FlightInstance instance) throws SQLException {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            pstmt.setTimestamp(2, combineDateAndTime(instance.getDepartureDate(), instance.getActualDepartureTime()));
            pstmt.setTimestamp(3, combineDateAndTime(instance.getDepartureDate(), instance.getActualArrivalTime()));
